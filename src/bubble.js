...
export class BubbleChart {
    ...
    updateTime(year){
        ...
        this.myBubbleChart.data.datasets.forEach((dataset, index) => {
            let country=dataset.label;
        dataset.data.forEach(element => {
                //TODO
                console.log("element");
                console.log(element);

                let data = this.bubbleData.filter(x => x.name==country)[0].value.filter(x => x.year == this.selectedYear);
                console.log("data");
                console.log(data);
                element.x=parseFloat(data[0].delta);
                element.y=parseFloat(data[0].co2);
                element.r= Math.sqrt(parseFloat(data[0].pop)/100000);
    ...
    updateData(bubbleData) {
        ...
        let all_countries=bubbleData.map(country => country.name);
        ...
        countries.forEach(displayedCountry => {
          //bubbleData.map()
          let country = displayedCountry.name;
        ...
        countries.forEach(newCountry => {
            ...
            this.myBubbleChart.data.datasets.push({
                ...
                data: [{
                    x: parseFloat(data[0].delta),
                    y: parseFloat(data[0].co2),
